...
impl<'a> IngestionPipeline<'a> {
    ...
    pub fn ingest_directory(&self, dir_path: &Path) -> Result<IngestionReport> {
        ...
        // TRACK-040: Scan all files for env var usage/definitions and populate config_registry.
    ...
    /// TRACK-040: Scan files for env var references and populate config_registry.
    ...
    pub fn ingest_file(&self, file_path: &Path) -> Result<usize> {
        // Read as raw bytes and decode lossily so non-UTF-8 files (Latin-1, Windows-1252, GBK, etc.)
...
mod tests {
    ...
    fn test_ingest_non_utf8_file_succeeds() {
        // A file containing Latin-1 bytes that are invalid UTF-8 must be ingested
        // successfully (with replacement characters) rather than counted as an error.
        // Use a .md extension so the crawler includes the file.
        let dir = TempDir::new().unwrap();
        let file = dir.path().join("latin1.md");
        // 0xE9 = 'é' in Latin-1, invalid as standalone byte in UTF-8.
