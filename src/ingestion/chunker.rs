...
fn try_parse_rust_item(line: &str, lines: &[&str], start: usize) -> Option<Chunk> {
    let is_fn = line.starts_with("pub fn ")
        ...
        || (line.starts_with("pub(") && (line.contains(") fn ") || line.contains(") async fn ")));

    let (name, node_type) = if is_fn {
...
fn is_ts_function_start(line: &str) -> bool {
    let is_decl_function = line.starts_with("export async function ")
    ...
    // For const-style definitions, require an arrow expression to avoid indexing
    // ordinary assignments like `const sid = buildSessionId(...)` as functions.
    let is_const_assignment = line.starts_with("export const ") || line.starts_with("const ");
    is_const_assignment && line.contains("=") && line.contains("=>")
}
...
mod tests {
    ...
    fn extract_fn_name_variants() {
        ...
        assert_eq!(
            extract_fn_name("pub(super) fn autocomplete_slash_input(input: &str)"),
            Some("autocomplete_slash_input".to_string())
        );
        assert_eq!(
            extract_fn_name("pub(crate) async fn process_event()"),
            Some("process_event".to_string())
        );
    }

    #[test]
    fn chunk_rust_indexes_restricted_visibility_fns() {
    ...
    #[test]
    fn chunk_typescript_avoids_const_assignment_false_positives() {
        let code = "const warning = formatWarning(errors[0]);\nfunction renderSreActions(pipeline) { return pipeline; }\nconst startSreTask = (pipeline) => pipeline;\n";
        let chunks = chunk_typescript(code);
        let names: Vec<String> = chunks.iter().map(|c| c.name.clone()).collect();

        assert!(names.iter().any(|n| n == "renderSreActions"));
        assert!(names.iter().any(|n| n == "startSreTask"));
        assert!(!names.iter().any(|n| n == "warning"));
    }

    #[test]
    fn chunk_typescript_supports_async_function_declarations() {
        let code = "async function execute_tool_call() { return 1; }\n";
        let chunks = chunk_typescript(code);
        assert_eq!(chunks.len(), 1);
        assert_eq!(chunks[0].name, "execute_tool_call");
    }
}
